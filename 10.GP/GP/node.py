...
ITER = 100
#in the data file, I added a header with the name of the variables
#which is in the form: value+variableNumber, so variable 1 will have the name value0
T = ["value" + str(x) for x in range(23)] + \
    ["c" + str(x) for x in range(10)]
P = [0.332, 0.332, 0.332, 0.002, 0.002]

#here are the operations that we need to use
operations = ['+', '-', '*', 'sin', 'cos']
#here we generated 10 random constants
constants = [random() for _ in range(10)]
...
class Node:
    ...
    def init(self, d):
        ...
        self.val = choice(operations, p=P)
        self.left = Node()
        self.left.init(d - 1)
    ...
    def __str__(self):
        if self.val in T:
            return str(constants[int(self.val[1])]) if self.val[0] == 'c' else str(self.val)
